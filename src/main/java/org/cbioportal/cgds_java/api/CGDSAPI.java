...
public class CGDSAPI {
	...
	public static CGDSRESTHelper<CGDSRESTManagerImpl> cgdsRESTHelper = new CGDSRESTHelper<CGDSRESTManagerImpl>(
			new CGDSRESTManagerImpl());

	public static <T extends AbstractResource> List<T> getListFromRESTResult(CGDSRESTResult<T> result) {
		List<T> list = null;
		if (result != null && result.getExitValue() == 0
				&& result.getClientResponse().getStatusInfo().getFamily().equals(Family.SUCCESSFUL)
